...
class IPEDSConfig(BaseModel):
    ...
    staging_dataset: str = Field("ipeds_staging", description="BigQuery staging dataset")
    mart_dataset: str = Field("ipeds_mart", description="BigQuery mart dataset")
...
class ConfigManager:
    ...
    def _load_config(self) -> Config:
        ...
        try:
            # Build config dictionary from parser
            config_dict: Dict[str, Dict[str, Any]] = {
                "paths": dict(self._parser.items("paths")),
                "ipeds": dict(self._parser.items("ipeds")),
                "bigquery": dict(self._parser.items("bigquery")),
                "logging": dict(self._parser.items("logging"))
            }

            # Convert specific fields to correct types
            if "ipeds" in config_dict and "default_year" in config_dict["ipeds"]:
                try:
                    # Only convert if it's a string
                    if isinstance(config_dict["ipeds"]["default_year"], str):
                        config_dict["ipeds"]["default_year"] = int(config_dict["ipeds"]["default_year"])
                except ValueError as e:
                    raise ValueError(f"Invalid configuration: {e}")

            return Config(
                paths=PathConfig(**config_dict["paths"]),
                ipeds=IPEDSConfig(**config_dict["ipeds"]),
